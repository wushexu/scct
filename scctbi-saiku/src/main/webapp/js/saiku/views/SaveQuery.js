...
var SaveQuery = Modal.extend({
    ...
    initialize: function(args) {
        ...
        if (args.query.name) {
            ...
            if(full_path.indexOf(Saiku.session.username + '/') != 0){
            	full_path = Saiku.session.username + '/'+full_path;
            }
    ...
    save: function(event) {
        ...
        */
        ...
        if (name != null && name.length > 0) {
        	if(name.indexOf('/') >= 0){
	        	if(name.indexOf(Saiku.session.username + '/') != 0){
	        		if(Saiku.session.roles.indexOf("ROLE_ADMIN") == -1){
	        			//foldername = Saiku.session.username + '/';
	        			alert('Cannot save querys to a folder belongs to others.\nYou can save to your own folder');
        		...
        		if(Saiku.session.roles.indexOf("ROLE_ADMIN") == -1){
        			alert('Cannot save querys to root folder.\nYou can save to your own folder');
        	        event.preventDefault();
        	        return false;
        		}
        	}
