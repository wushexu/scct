        ...
        $.contextMenu({
                ...
                events: {
                    show: function(opt) {
                        ...
                        var othersFolder=false;
                    	if(path.indexOf('/') >= 0){
            	        	if(path.indexOf(Saiku.session.username + '/') != 0){
                        ...
                        if (othersFolder || (typeof item.acl != "undefined" && _.indexOf(item.acl, "WRITE") <  0)) {
                    ...
                    if (key == "open" && $(this).hasClass('query')) {
                    	...
                    	if(path.indexOf('/') >= 0){
            	        	if(path.indexOf(Saiku.session.username + '/') != 0){
            	        		if(Saiku.session.roles.indexOf("ROLE_ADMIN") == -1){
            	        			//othersFolder:
            	        			alert('Cannot delete querys belongs to others');
            	        			return false;
            	        		}
            	        	}
                    	} else {
        	        		if(Saiku.session.roles.indexOf("ROLE_ADMIN") == -1){
        	        			//root folder:
        	        			alert('Cannot delete querys from root folder');
        	        			return false;
        	        		}
                    	}
