...
var SelectionsModal = Modal.extend({
    ...
    populate: function(model, response) {
            ...
            var datePat=/^20\d\d-\d\d?-\d\d?$/;
            function processCaption(uniqueName,caption){
                //[POD].[2014].[Q1]
                //[POD.Financial].[2013].[43]
            	//[CreatedDate].[2014].[Q1]
            	//[UpliftDate].[2014].[Q1]
            	//[CarrPickupDate].[2014].[Q1]
            	if(datePat.test(caption)){
            		return caption;
            	}
                if(uniqueName.indexOf('[POD].')==0||uniqueName.indexOf('[CDD].')==0){
                	caption=uniqueName.substr(6);
                }else if(uniqueName.indexOf('[POD.Financial].')==0||uniqueName.indexOf('[CDD.Financial].')==0){
                	caption=uniqueName.substr(16);
                }else if(uniqueName.indexOf('[CreatedDate].')==0){
                	caption=uniqueName.substr(14);
                }else if(uniqueName.indexOf('[UpliftDate].')==0){
                	caption=uniqueName.substr(13);
                }else if(uniqueName.indexOf('[CarrPickupDate].')==0){
                	caption=uniqueName.substr(17);
                }
                return caption;
            }
            ...
            for (var j = 0, len = this.selected_members.length; j < len; j++) {
                ...
                if (member.levelUniqueName == decodeURIComponent(this.member.level) && 
                    ...
                    var caption=processCaption(uniqueName,member.caption);
                    selected_members_opts += '<option value="' + encodeURIComponent(uniqueName) + '">' + caption + "</option>";
            ...
            for (var i = 0, len = this.available_members.length; i < len; i++) {
                ...
                var uniqueName=member.uniqueName;
                var caption=processCaption(uniqueName,member.caption);
                available_members_opts += '<option value="' + encodeURIComponent(uniqueName) + '">' + caption + "</option>";
            }
