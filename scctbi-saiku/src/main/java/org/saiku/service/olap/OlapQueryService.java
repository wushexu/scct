...
public class OlapQueryService implements Serializable {
    ...
    public ResultSet drillthrough(String queryName, Integer maxrows,
    		String returns) {
    	
    	return drillthrough(queryName, maxrows, returns, null);
    }

    public ResultSet drillthrough(String queryName, int maxrows, String returns,
    		DrillthroughPreProcessor mdxPreProcessor) {
        OlapStatement stmt = null;
        try {
            final OlapConnection con = olapDiscoverService.getNativeConnection(getQuery(queryName).getCube().getConnectionName());
            stmt = con.createStatement();
            ...
            if(mdxPreProcessor != null) {
            	String[] mdxAndReturns = mdxPreProcessor.process(mdx, returns);
            	mdx = mdxAndReturns[0];
            	returns = mdxAndReturns[1];
            }
    ...
    public ResultSet drillthrough(String queryName, List<Integer> cellPosition, Integer maxrows,
    	...
    	return drillthrough(queryName, cellPosition, maxrows, returns, null);
    }

    public ResultSet drillthrough(String queryName, List<Integer> cellPosition, Integer maxrows,
    		String returns, DrillthroughPreProcessor mdxPreProcessor) {
        ...
        try {
            ...
            select = buf.toString();
            if(mdxPreProcessor != null) {
            	String[] mdxAndReturns = mdxPreProcessor.process(select, returns);
            	select = mdxAndReturns[0];
            	returns = mdxAndReturns[1];
            }
